...
@section('title', __('Alerts List'))

@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('Alerts List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.alerts.store') }}" aria-label="create_page_link"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "alerts", {
            Search: "{{ route('actions.alerts.search') }}",
            Patch: "{{ route('views.alerts.patch', 'XXX') }}",
            Clear: "{{ route('actions.alerts.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection

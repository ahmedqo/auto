...
@section('title', __('Models List'))

@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('Models List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.models.store') }}" aria-label="create_page_link"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "models", {
            Search: "{{ route('actions.models.search') }}",
            Patch: "{{ route('views.models.patch', 'XXX') }}",
            Clear: "{{ route('actions.models.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection

...
@section('title', __('BlackList'))

@section('content')
    <div class="flex flex-col gap-2">
        <neo-datavisualizer print search filter download title="{{ __('BlackList') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.blacklist.store') }}"
...
@section('scripts')
    <script>
        TableVisualizer(document.querySelector("neo-datavisualizer"), "blacklist", {
            Search: "{{ route('actions.blacklist.search') }}",
            Patch: "{{ route('views.blacklist.patch', 'XXX') }}",
            Clear: "{{ route('actions.blacklist.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
